...
public class AutoTemperatureChart extends View {
    ...
    public AutoTemperatureChart(Context context) {
        super(context);
    }

    public AutoTemperatureChart(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public AutoTemperatureChart(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }


    //    public AutoTemperatureChart(Context context, String[] xLineData, String[] value) {
//        super(context);
...
//    }
    ...
    public AutoTemperatureChart(Context context, List<TiWen> temperatureList) {
        super(context);
        this.context = context;
        choose(temperatureList);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < valueList.size(); i++) {
            ...
            canvas.drawText(xRQData, padding_left + i * perWidth - textWidthRQ / 2, yPoint + 30, paintXYText);
            canvas.drawText(xSJdata, padding_left + i * perWidth - textWidthSJ / 2, yPoint + 50, paintXYText);
        }
        //画X轴单位
        ...
        for (int i = 0; i < yLineData.length; i++) {
            ...
            //画Y轴文字
            canvas.drawText(yLineData[i], xPoint - 30, yPoint - i * perHeight, paintXYText);
        }
        //画Y轴单位
        canvas.drawText("(℃)", xPoint - 30, yPoint - (yLineData.length - 1) * perHeight - 20, paintXYText);
        ...
        for (int i = 0; i < valueList.size(); i++) {
            ...
            //画温度圆圈
            canvas.drawCircle(xPos, yPos, 9, paintContent);
            //画圆圈上数字
            canvas.drawText(temperatureData, xPos, yPos - 15, paintContentText);
