...
public class RcyMoreAdapter extends CommonAdapter<CbMoreBean> {
    ...
    protected void convert(ViewHolder holder, final CbMoreBean bean, int position) {
        ...
        if (bean.getTag().equals("0")) {
            ...
            if(bean.getTitle().equals("疾病因素")||bean.getTitle().equals("使用药物")||bean.getTitle().equals("感官因素")||bean.getTitle().equals("部位")||bean.getTitle().equals("性质")||bean.getTitle().equals("伴随症状")||bean.getTitle().equals("物理治疗")||bean.getTitle().equals("引导治疗")){
                GvCbOtherAdapter gvAdapter = new GvCbOtherAdapter(mContext, bean.getCbMoreBean(), R.layout.gv_item_checkbox);
                gridview.setAdapter(gvAdapter);
                gvAdapter.setCheckListener(new GvCbOtherAdapter.OnCheckClickListener() {
                    @Override
                    public void onCheckClick(String pfxx, int sorce, int position, String content) {
                        if (mListener != null) {
                            mListener.onCheckClick(pfxx, sorce, position, bean.getTitle(),content);
                        }
                    }
                });
            ...
            if(bean.getTitle().equals("高危导管(Ⅰ类导管)")||bean.getTitle().equals("非高危导管(Ⅱ类导管)")){
                LvCbOtherAdapter lvAdapter = new LvCbOtherAdapter(mContext, bean.getCbMoreBean(), R.layout.lv_item_checkbox);
                listview.setAdapter(lvAdapter);
                lvAdapter.setCheckListener(new LvCbAdapter.OnCheckClickListener() {
                    @Override
                    public void onCheckClick(String pfxxTag, int sorce, int position, String content) {
                        if (mListener != null) {
                            mListener.onCheckClick(pfxxTag, sorce, position, bean.getTitle(),content);
                        }
                    }
    ...
    public interface OnCheckClickListener {
        void onCheckClick(String pfxxTag, int sorce, int position, String title, String contennt);
    }
