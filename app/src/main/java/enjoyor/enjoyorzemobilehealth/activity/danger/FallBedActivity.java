...
public class FallBedActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private String[] endNames = new String[]{"A.卧床休息、留陪人", "B.根据医嘱使用保护性约束带", "C.行走、活动、如厕时需搀扶;头眩晕时避免上、下床", "D.根据医嘱关注血压及血糖的变化", "E.告知病人及家属药物的副作用及注意事项", "F.保持地面干燥;光线充足;有障碍物时要防止标识牌",
            "G.嘱病人穿大小合适的拖鞋", "HuLiJi.使用带轮子的设施、设备都要锁定轮子,防止滑动。设施、设备出现故障及时报修", "I.告知病人及家属卧床时床档必须在功能位,教给病人及家属床档的正确方法", "J.根据需要床旁放置便盆或尿壶,沐浴时放置座椅,如厕时使用扶手,有情况时使用紧急呼叫器"};
    private int all = 0;
            ...
            public void onCheckClick(String pfxxTag, int sorce, int position, String title, String content) {
                switch (title) {
                    case "年龄":
                        sorces[0] = sorce;
                        pfxxTags[0] = pfxxTag;
                        contents[0] = "("+String.valueOf(position+1)+")";
                        break;
                    case "既往史":
                        sorces[1] = sorce;
                        pfxxTags[1] = pfxxTag;
                        contents[1] = "("+String.valueOf(position+1)+")";
                        break;
                    case "意识状态":
                        sorces[2] = sorce;
                        pfxxTags[2] = pfxxTag;
                        contents[2] = "("+String.valueOf(position+1)+")";
                        break;
                    case "感官因素":
                        sorces[3] = sorce;
                        pfxxTags[3] = pfxxTag;
                        contents[3] = "("+String.valueOf(position+1)+")";
                        break;
                    case "使用药物":
                        sorces[4] = sorce;
                        pfxxTags[4] = pfxxTag;
                        contents[4] = "("+String.valueOf(position+1)+")";
                        break;
                    case "疾病因素":
                        sorces[5] = sorce;
                        pfxxTags[5] = pfxxTag;
                        contents[5] = "("+String.valueOf(position+1)+")";
                        break;
                    default:
                        break;
                }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.sj:
                ...
                //时间选择器
                TimePickerView pvTime = new TimePickerView.Builder(FallBedActivity.this, new TimePickerView.OnTimeSelectListener() {
                    @Override
    ...
    private void netWorkTime() {
        getCurDate(tvPingguBed.getText().toString().trim());
        //一共4个参数：病人住院ID，病区ID，评估内容，提醒时间
//        示例：20111201¤0404¤压疮¤2017/6/14 15:33:24
        String canshu = brzyid + f + bqId + f + "跌倒坠床风险评估" + f + date;
        //网络参数设置
        Log.e("添加时间提醒", canshu);
    ...
    private void netWorkUpdata() {
        ...
        String canshu = brzyid + f + shijian + f + allscore + f + fenxian + f + allString + f + pingguren + f + shijian + f + "2" + f + "" + f + sqsh + f + rydate + f + name + f + brid
                + f + sex + f + age + f + bqId + f + bqmc + f + lastPfMsg + f + cuoShimsg + f+pfContent+ f + itemId;
        //网络参数设置
        Log.e("修改参数", canshu);
        zhierCall = (new ZhierCall())
                .setId(uesrid)
    ...
    private void netWorkAdd() {
        ...
        if(lastPfMsg==""){
            ...
            for(int i=0;i<str1.length();i++){
                if(str1.substring(i,i+1).equals("1")){
                    pfContent=pfContent+"("+String.valueOf(i+1)+")";
                }
            }
            pfContent=pfContent+"#";
            for(int i=0;i<str2.length();i++){
        ...
        String canshu = brzyid + f + shijian + f + allscore + f + fenxian + f + allString + f + pingguren + f + shijian + f + "2" + f + "" + f + sqsh + f + rydate + f + name + f + brid
                + f + sex + f + age + f + bqId + f + bqmc + f + lastPfMsg + f + cuoShimsg+ f + pfContent;
        //网络参数设置
        Log.e("保存参数", canshu);
        zhierCall = (new ZhierCall())
                .setId(uesrid)
