package enjoyor.enjoyorzemobilehealth.entities;

/**
 * ���������ؼ�ֵ
 * 
 * @author Administrator xuhaijiang
 * 
 */
public class ValueSMTZKongJian{

	public ValueSMTZKongJian() {

	}

	public ValueSMTZKongJian(String KongJianMC) {
		this.KongJianMC = KongJianMC;
	}

	private String DR;
	private String ShengMingTZID;
	private String JiChuXiangMuID;
	private String KongJianMC;
	private String ShuZhi1;
	private String ShuZhi2;

	private String ShuZhiDW;
	private String JiLuSJ;
	private String CaiJiRen;
	private String CaiJiRQ;
	private String CaiJiSJ;
	private String XiuGaiRen;

	private String XiuGaiSJ;
	private String XiuGaiBZ;
	private String BingQuID;
	private String BingRenZYID;
	private String BingRenXM;
	private String PanDuanBZ;

	public String getDR() {
		return DR == null ? "" : DR;
	}

	public void setDR(String DR) {
		this.DR = DR;
	}

	public String getShengMingTZID() {
		return ShengMingTZID == null ? "" : ShengMingTZID;
	}

	public void setShengMingTZID(String shengMingTZID) {
		ShengMingTZID = shengMingTZID;
	}

	public String getJiChuXiangMuID() {
		return JiChuXiangMuID == null ? "" : JiChuXiangMuID;
	}

	public void setJiChuXiangMuID(String jiChuXiangMuID) {
		JiChuXiangMuID = jiChuXiangMuID;
	}

	public String getKongJianMC() {
		return KongJianMC == null ? "" : KongJianMC;
	}

	public void setKongJianMC(String kongJianMC) {
		KongJianMC = kongJianMC;
	}

	public String getShuZhi1() {
		return ShuZhi1 == null ? "" : ShuZhi1;
	}

	public void setShuZhi1(String shuZhi1) {
		ShuZhi1 = shuZhi1;
	}

	public String getShuZhi2() {
		return ShuZhi2 == null ? "" : ShuZhi2;
	}

	public void setShuZhi2(String shuZhi2) {
		ShuZhi2 = shuZhi2;
	}

	public String getShuZhiDW() {
		return ShuZhiDW == null ? "" : ShuZhiDW;
	}

	public void setShuZhiDW(String shuZhiDW) {
		ShuZhiDW = shuZhiDW;
	}

	public String getJiLuSJ() {
		return JiLuSJ == null ? "" : JiLuSJ;
	}

	public void setJiLuSJ(String jiLuSJ) {
		JiLuSJ = jiLuSJ;
	}

	public String getCaiJiRen() {
		return CaiJiRen == null ? "" : CaiJiRen;
	}

	public void setCaiJiRen(String caiJiRen) {
		CaiJiRen = caiJiRen;
	}

	public String getCaiJiRQ() {
		return CaiJiRQ == null ? "" : CaiJiRQ;
	}

	public void setCaiJiRQ(String caiJiRQ) {
		CaiJiRQ = caiJiRQ;
	}

	public String getCaiJiSJ() {
		return CaiJiSJ == null ? "" : CaiJiSJ;
	}

	public void setCaiJiSJ(String caiJiSJ) {
		CaiJiSJ = caiJiSJ;
	}

	public String getXiuGaiRen() {
		return XiuGaiRen == null ? "" : XiuGaiRen;
	}

	public void setXiuGaiRen(String xiuGaiRen) {
		XiuGaiRen = xiuGaiRen;
	}

	public String getXiuGaiSJ() {
		return XiuGaiSJ == null ? "" : XiuGaiSJ;
	}

	public void setXiuGaiSJ(String xiuGaiSJ) {
		XiuGaiSJ = xiuGaiSJ;
	}

	public String getXiuGaiBZ() {
		return XiuGaiBZ == null ? "" : XiuGaiBZ;
	}

	public void setXiuGaiBZ(String xiuGaiBZ) {
		XiuGaiBZ = xiuGaiBZ;
	}

	public String getBingQuID() {
		return BingQuID == null ? "" : BingQuID;
	}

	public void setBingQuID(String bingQuID) {
		BingQuID = bingQuID;
	}

	public String getBingRenZYID() {
		return BingRenZYID == null ? "" : BingRenZYID;
	}

	public void setBingRenZYID(String bingRenZYID) {
		BingRenZYID = bingRenZYID;
	}

	public String getBingRenXM() {
		return BingRenXM == null ? "" : BingRenXM;
	}

	public void setBingRenXM(String bingRenXM) {
		BingRenXM = bingRenXM;
	}

	public String getPanDuanBZ() {
		return PanDuanBZ == null ? "" : PanDuanBZ;
	}

	public void setPanDuanBZ(String panDuanBZ) {
		PanDuanBZ = panDuanBZ;
	}

	//	public String getDR() {
//		return DR;
//	}
//
//	public void setDR(String dR) {
//		DR = dR;
//	}
//
//	public String getShengMingTZID() {
//		return ShengMingTZID;
//	}
//
//	public void setShengMingTZID(String shengMingTZID) {
//		ShengMingTZID = shengMingTZID;
//	}
//
//	public String getJiChuXiangMuID() {
//		return JiChuXiangMuID;
//	}
//
//	public void setJiChuXiangMuID(String jiChuXiangMuID) {
//		JiChuXiangMuID = jiChuXiangMuID;
//	}
//
//	public String getKongJianMC() {
//		return KongJianMC;
//	}
//
//	public void setKongJianMC(String kongJianMC) {
//		KongJianMC = kongJianMC;
//	}
//
//	public String getShuZhi1() {
//		return ShuZhi1;
//	}
//
//	public void setShuZhi1(String shuZhi1) {
//		ShuZhi1 = shuZhi1;
//	}
//
//	public String getShuZhi2() {
//		return ShuZhi2;
//	}
//
//	public void setShuZhi2(String shuZhi2) {
//		ShuZhi2 = shuZhi2;
//	}
//
//	public String getShuZhiDW() {
//		return ShuZhiDW;
//	}
//
//	public void setShuZhiDW(String shuZhiDW) {
//		ShuZhiDW = shuZhiDW;
//	}
//
//	public String getJiLuSJ() {
//		return JiLuSJ;
//	}
//
//	public void setJiLuSJ(String jiLuSJ) {
//		JiLuSJ = jiLuSJ;
//	}
//
//	public String getCaiJiRen() {
//		return CaiJiRen;
//	}
//
//	public void setCaiJiRen(String caiJiRen) {
//		CaiJiRen = caiJiRen;
//	}
//
//	public String getCaiJiRQ() {
//		return CaiJiRQ;
//	}
//
//	public void setCaiJiRQ(String caiJiRQ) {
//		CaiJiRQ = caiJiRQ;
//	}
//
//	public String getCaiJiSJ() {
//		return CaiJiSJ;
//	}
//
//	public void setCaiJiSJ(String caiJiSJ) {
//		CaiJiSJ = caiJiSJ;
//	}
//
//	public String getXiuGaiRen() {
//		return XiuGaiRen;
//	}
//
//	public void setXiuGaiRen(String xiuGaiRen) {
//		XiuGaiRen = xiuGaiRen;
//	}
//
//	public String getXiuGaiSJ() {
//		return XiuGaiSJ;
//	}
//
//	public void setXiuGaiSJ(String xiuGaiSJ) {
//		XiuGaiSJ = xiuGaiSJ;
//	}
//
//	public String getXiuGaiBZ() {
//		return XiuGaiBZ;
//	}
//
//	public void setXiuGaiBZ(String xiuGaiBZ) {
//		XiuGaiBZ = xiuGaiBZ;
//	}
//
//	public String getBingQuID() {
//		return BingQuID;
//	}
//
//	public void setBingQuID(String bingQuID) {
//		BingQuID = bingQuID;
//	}
//
//	public String getBingRenZYID() {
//		return BingRenZYID;
//	}
//
//	public void setBingRenZYID(String bingRenZYID) {
//		BingRenZYID = bingRenZYID;
//	}
//
//	public String getBingRenXM() {
//		return BingRenXM;
//	}
//
//	public void setBingRenXM(String bingRenXM) {
//		BingRenXM = bingRenXM;
//	}
//
//	public String getPanDuanBZ() {
//		return PanDuanBZ;
//	}
//
//	public void setPanDuanBZ(String panDuanBZ) {
//		PanDuanBZ = panDuanBZ;
//	}

}
